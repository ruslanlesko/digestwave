...
public class RatingService {
	...
	public RatingService(PostRepository postRepository, TopPostRepository topPostRepository,
			OllamaAPI ollama, @Value("${app.model}") String model, @Qualifier("prompt") String prompt) {
	...
	public List<TopPost> ratePosts(String topic, String region) {
		List<Post> posts = postRepository.findTop20ByTopicAndRegionOrderByPublicationTimeDesc(topic, region);
		...
		if (topPosts.size() < 3) {
			throw new OllamaUnavailableException("Cannot generate proper result");
		}

		topPostRepository.deleteAll(topPostRepository.findAllByTopicAndRegion(topic, region));
	...
	@Transactional
	public void saveTopPosts(List<TopPost> topPosts) {
	...
	private List<TopPost> queryTopPosts(List<Post> posts) {
		try {
			...
			OllamaResult result = ollama.generate(model, prompt, new OptionsBuilder().setTemperature(0.25f).build());
			if (result.getHttpStatusCode() != 200) {
				throw new OllamaUnavailableException("Cannot query ollama, status code " + result.getHttpStatusCode());
			}
